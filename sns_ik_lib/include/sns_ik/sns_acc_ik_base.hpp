...
protected:
   ...
   */
  bool checkVelBnd(const Eigen::VectorXd& ddqUpp);

  /*
   * Solve the following equation for the variable ddqUpp:
   *    J * W * (ddqUpp - dqNull) = dx - J*dqNull
  ...
  bool solveProjectionEquation(const Eigen::MatrixXd& J, const Eigen::MatrixXd& JW,
                               ...
                               Eigen::VectorXd* ddqUpp, double* resErr);
  ...
  ExitCode computeTaskScalingFactor(const Eigen::MatrixXd& J, const Eigen::MatrixXd& JW,
                                const Eigen::VectorXd& dx, const Eigen::VectorXd& ddqUpp,
